...
function isCustomResolution(res: Resolution): res is { Custom: { width: number; height: number } } {
  return typeof res === "object" && "Custom" in res;
}

function isSpecificEncoder(enc: EncoderPref): enc is { Specific: string } {
  return typeof enc === "object" && "Specific" in enc;
}
...
class RulesTab {
  ...
  static renderTable(): void {
    ...
    this.rules.forEach((rule) => {
      ...
      let resVal = "";
      if (res === "P720") resVal = "720p";
      else if (res === "P1080") resVal = "1080p";
      else if (res === "P2160") resVal = "2160p";
      else if (isCustomResolution(res)) resVal = `${res.Custom.width}x${res.Custom.height}`;

      const enc = rule.preferred_encoder;
      let encVal = "自动";
      if (isSpecificEncoder(enc)) encVal = enc.Specific;
      ...
      html += `<tr>
        ...
        <td>
          <select class="rule-select" data-id="${rule.id}" data-field="preferred_encoder">
            <option value="auto" ${enc === "Auto" ? "selected" : ""}>自动</option>
  ...
  static onFieldChange(el: HTMLElement): void {
    ...
    if (el instanceof HTMLSelectElement) {
      const val = el.value;
      if (field === "resolution") {
        if (val === "720p") rule.resolution = "P720";
        else if (val === "1080p") rule.resolution = "P1080";
        else if (val === "2160p") rule.resolution = "P2160";
        else if (val === "custom") {
        ...
        rule.preferred_encoder = val === "auto" ? "Auto" : { Specific: val };
      }
    } else if (el instanceof HTMLInputElement) {
      const numFields = ["bitrate_threshold_kbps", "target_bitrate_kbps"];
